...
@section('titulo', 'Nuevo anuncio | ' . $tituloSeccion)

@section('contenido')
<main id="main" class="main">
  <div class="pagetitle"><h1>Nuevo anuncio de {{ $tituloSeccion }}</h1><p>Comparte una actividad o recuerdo con {{ $tituloSeccion }}.</p></div>
  <section class="section"><div class="card"><div class="card-body pt-4">
    <form action="{{ route('anuncios.store') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="mt-4"><button class="btn btn-primary" type="submit"><i class="bi bi-cloud-arrow-up"></i> Publicar anuncio</button> <a class="btn btn-light" href="{{ $seccion === 'escuela_dominical' ? route('anuncios.escuela.index') : route('anuncios.index') }}">Cancelar</a></div>
    </form>
  </div></div></section>
</main>
@endsection